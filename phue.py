...
PhueExceptionStrings = {
    7: 'Unknown username',
    128: 'No IP address available'
}
...
class PhueException(Exception):
    ...
    def __init__(self, id, message):
        ...
        print "Exception %i: %s" % (id, message)
...
class Bridge(object):
    ...
    def request(self, mode='GET', address=None, data=None):
        ...
        if self.ip is None:
            error_type = 128
            raise PhueException(error_type, PhueExceptionStrings[error_type])
    ...
    def get_local_bridges(self, set_result=False):
        """ Get the bridge info from the meethue.com nupnp api """
    ...
    def get_ip_address(self, set_result=False):
        ...
        """ Get the bridge ip address from the local bridges """

        data = self.get_local_bridges()
